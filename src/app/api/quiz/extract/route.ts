...
const ExtractQuizSchema = z.object({
  files: z.array(
    z.object({
      id: z.string(),
      name: z.string(),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validated = ExtractQuizSchema.safeParse(body);
    ...
    let title = `Extracted Quiz from ${files[0]?.name || "Files"}`;
    let description = "Extracted questions from existing quiz content";
    ...
    try {
      ...
      const titleDescResult = await generateQuizTitleDescription({
        ...
        isExtractMode: true, // Always true for extract mode
        targetLanguage:
          settings.language === "AUTO"
            ? "auto"
            : settings.language.toLowerCase(),
        filename: files[0]?.name,
        category: undefined, // Can be added if available in settings
    ...
    console.log(
      "✅ Quiz extracted successfully with AI:",
    ...
    console.error("❌ Failed to extract quiz:", error);
