...
export function TextContentUploader({
  ...
  const isBusy = isCreatingFlashcards;
  ...
  const handleCreateFlashcards = async () => {
    ...
    onProcessingStart?.(
      "Text Content",
      t("create.textContent.extractingFlashcards"),
    );

    try {
      // TODO: Implement flashcard creation from text content
      // await extractFlashcardsFromText(textContent);

      // Simulate processing
      await new Promise((resolve) => setTimeout(resolve, 2000));

      onProcessingDone?.(true);
      // TODO: Navigate to flashcard edit page
    } catch (error) {
        ...
        <div className="flex gap-2">
          ...
          >
            {isCreatingFlashcards ? (
              ...
              <>{t("create.textContent.extractFlashcards")}</>
            )}
          </Button>
        </div>
      </div>
